...
document.querySelector('.todo_app').insertBefore(statusMessage, tasksDiv);
...
const removeTask = (taskValue) => {
    ...
    displayTasks();
    updateStatusMessage("Task deleted successfully", "success");
};
...
document.querySelector("#delete_all").addEventListener("click", () => {
    localStorage.clear();
    displayTasks();
    updateStatusMessage("All tasks deleted successfully", "success");
});
...
document.querySelector("#add_task").addEventListener("click", () => {
    ...
    if (newTaskInput.value.length == 0) {
        updateStatusMessage("Please add a task", "error");
    } else {
        if (updateNote == "") {
            updateStorage(count, newTaskInput.value, false);
            updateStatusMessage("Task added successfully", "success");
        } else {
            let existingCount = updateNote.split("_")[0];
            removeTask(updateNote);
            updateStorage(existingCount, newTaskInput.value, false);
            updateNote = "";
            updateStatusMessage("Task updated successfully", "success");
        }
        count += 1;
        newTaskInput.value = "";
    }
